...
export default function CartContent({ onClose, isDrawer = false }) {
  ...
  if (cartItems.length === 0) {
    return (
      <div
        className={`flex flex-col items-center justify-center text-gray-500 space-y-4 py-10 ${isDrawer ? "h-full" : "h-64"}`}
      >
        ...
        {isDrawer && onClose && (
          ...
          >
            <Translation en="Start browsing" bn="ব্রাউজ করা শুরু করুন" />
          </button>
        )}
      </div>
    );
